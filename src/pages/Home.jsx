...
export default function Home() {
  ...
  return (
    <div style={{ width: '100%', minHeight: '80vh', color: textColor }}>
      <div style={{ position: 'relative', zIndex: 2, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '40px 20px', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '15px', flexWrap: 'wrap', justifyContent: 'center' }}>
          <Link to="/market" style={{ ...btnBase, background: textColor, color: theme === 'dark' ? '#000' : '#fff' }}>Rynek</Link>
          <Link to="/kontakt" style={{ ...btnBase, border: '2px solid gray', background: 'transparent', color: 'inherit' }}>Kontakt</Link>
        </div>
      </div>
    </div>
  )
...
function Karta({ nazwa, dane, godziny, kolor, bg }) {
  return (
    <div style={{ background: bg, padding: '20px', borderRadius: '12px', minWidth: '200px', maxWidth: '280px', border: `1px solid ${dane.czyOtwarta ? kolor : 'gray'}`, boxShadow: '0 4px 6px rgba(0,0,0,0.1)' }}>
      ...
      <div style={{ color: dane.czyOtwarta ? kolor : '#ef4444', fontWeight: 'bold', margin: '5px 0' }}>{dane.czyOtwarta ? '🟢 OTWARTA' : '🔴 ZAMKNIĘTA'}</div>
